...
이 구현은 `SymbolWithScope`에서 제공하는 기능을 활용하며, 네임스페이스가 스코프로서 작동하게 하고, 동시에 다른 심볼들(변수, 함수 등)을 포함할 수 있도록 합니다. 중첩 네임스페이스는 각 네임스페이스 인스턴스가 자신의 스코프 내에 다른 네임스페이스 인스턴스를 포함함으로써 구현됩니다.
