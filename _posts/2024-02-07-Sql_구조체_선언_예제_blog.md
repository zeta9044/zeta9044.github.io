...
여기서 `source_file.c`는 전처리하려는 소스 파일의 이름입니다. `-E` 옵션은 컴파일러에게 전처리만 수행하고 그 결과를 표준 출력으로 보내라고 지시합니다.

예를 들어, 제공된 코드가 `example.c` 파일에 저장되어 있다고 가정하면, 다음 명령어를 사용합니다:

```sh
gcc -E example.c
```

이 명령어는 매크로가 확장되고, 포함된 파일이 처리된 후의 코드를 출력합니다. 출력 결과는 `#include`, `#define` 등의 전처리 지시문이 처리된 상태를 반영합니다. 
...
이렇게 하면 전처리 결과가 `preprocessed_example.c` 파일에 저장됩니다. 전처리된 코드를 검토하면 `DEFINE_STRUCTURE(Employee)` 매크로가 확장되어 `Employee` 구조체의 실제 정의를 볼 수 있으며, 이는 소스 코드에 직접 작성했던 것과 동일한 형태일 것입니다.
