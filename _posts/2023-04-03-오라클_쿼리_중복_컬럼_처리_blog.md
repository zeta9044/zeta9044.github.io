...
하지만 이 쿼리에서는 `CASE` 구문에서 모든 조건이 항상 `true`로 평가되므로, `ORDER BY` 구문은 실제로 `a`, `b`, `c`, `d`, `e`와 같은 컬럼 이름만 사용하는 것과 같아집니다. 따라서 `SELECT a, b, c, d, e, z, y FROM X ORDER BY a, b, c, d, e`와 같이 단순화된 쿼리를 사용하는 것이 더 효율적일 수 있습니다.
