...
Git for Windows에 포함된 Git Bash를 사용하여 리눅스 스타일의 Bash 스크립트를 실행할 수도 있습니다. 이 경우, 위에서 제공된 리눅스용 Bash 스크립트를 `.git/hooks` 디렉토리에 `post-checkout`라는 이름으로 저장하고 실행 권한을 부여하면 됩니다.
